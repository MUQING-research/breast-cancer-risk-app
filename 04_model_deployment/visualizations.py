# Module guide:
# - Role: Provide reusable deployment plotting helpers.
# - Workflow: Turn precomputed bundle diagnostics into publication-style figures.
# - Design note: Never require raw patient-level training tables in this module.
...
# Inputs: title, wide. Outputs and side effects follow the routine contract.
...
# Inputs: fig, caption. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Inputs: fig. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def png(fig):
    buffer = io.BytesIO()
...
# Inputs: ax, **kwargs. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: roc_figure is responsible for perform figure.
# Inputs: data. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def roc_figure(data):
...
# Function guide: calibration_figure is responsible for perform figure.
# Inputs: data. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def calibration_figure(data):
    ...
    for label, target, probability, brier, color, style in (
        ...
        predicted, observed, _ = _quantile_calibration_points(
...
# Inputs: data, feature. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: cv_figure is responsible for perform figure.
# Inputs: data. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def cv_figure(data):
...
# Function guide: linearity_figure is responsible for perform figure.
# Inputs: data, feature. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: vif_figure is responsible for perform figure.
# Inputs: data. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def vif_figure(data):
    names = data["SEL_COLS"]
    ...
    legend(ax, loc="lower right")
    return finish(fig, "WDBC training | Raw inputs. VIF for the linear design terms used by the final model.")


# Function guide: threshold_figure is responsible for perform figure.
# Inputs: data, threshold. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
