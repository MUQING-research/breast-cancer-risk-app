...
def screen_variables(X_train, y_train):
    """Screen candidates and functional forms without accessing test rows."""
    ...
    for name in X_train.columns:
        ...
        decisions[name] = {
            ...
            'decision_rationale': 'Transformation requires improved skewness and linear likelihood; form follows the prespecified GAM edf/p rule, with LRT reported independently.',
        }
        summaries.append({
...
def make_design(selected_features, decisions):
    """Return a sklearn transformer containing no patient-level records."""
    transformers = []
    for index, name in enumerate(selected_features):
        decision = decisions[name]
        steps = [('transform', transformation(decision['transformation']))]
        if decision['functional_form'] == 'quadratic':
            from sklearn.preprocessing import StandardScaler
            steps += [('center', StandardScaler(with_std=False)),
                      ('quadratic', PolynomialFeatures(degree=2, include_bias=False))]
        elif decision['functional_form'] == 'spline':
            steps += [('spline', SplineTransformer(
                n_knots=3, degree=3, include_bias=False, extrapolation='linear'))]
