# Module guide:
# - Role: Generate documentation figures from saved model artifacts.
# - Workflow: Load derived bundle data and write deterministic static assets.
# - Design note: Do not refit the deployed model while generating documentation.
...
BUNDLE_PATH = ROOT / "04_model_deployment" / "breast_cancer_model_bundle.pkl"
ASSET_DIR = Path(__file__).resolve().parent / "assets"


# Function guide: _load_bundle is responsible for load the persisted model bundle.
...
# Inputs: ax. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: _panel_title is responsible for perform title.
# Inputs: ax, letter: str, title: str. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Inputs: fig, filename: str, *, use_tight_layout: bool. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: _display_name is responsible for display name.
# Inputs: feature: str. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def _display_name(feature: str) -> str:
    return feature.replace("worst ", "").capitalize()


# Function guide: _quantile_calibration_points is responsible for perform calibration points.
# Inputs: the component state. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Inputs: bundle: dict. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def _model_performance_figure(bundle: dict) -> None:
    ...
    _panel_title(ax, "C", "Fitted linear coefficients")
...
# Function guide: _feature_selection_figure is responsible for perform selection figure.
# Inputs: bundle: dict. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
...
# Function guide: _linearity_figure is responsible for perform figure.
# Inputs: bundle: dict. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def _linearity_figure(bundle: dict) -> None:
    ...
    overview_ax = axes.flat[-1]
    _style_axis(overview_ax)
    overview_ax.set_xticks([])
    overview_ax.set_yticks([])
    _panel_title(overview_ax, "H", "Diagnostic overview")
    nonlinear = [
        feature
...
# Function guide: _vif_figure is responsible for perform figure.
# Inputs: bundle: dict. Outputs and side effects follow the routine contract.
# Keep this boundary focused on one workflow step so it remains easy to test and reuse.
def _vif_figure(bundle: dict) -> None:
    ...
    _panel_title(ax, "A", "Collinearity among processed retained inputs")
